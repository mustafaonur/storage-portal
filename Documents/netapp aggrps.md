...
# ============================================================
# OS RESOLVER (host name patterns - hem Huawei hem PMAX kullanir)
# ============================================================
function Resolve-HostOS {
    ...
    if (-not $HostName) { return $CurrentOs }
    $name = $HostName.ToUpper()

    # Mevcut OS guvenilir mi? (Generic, Unknown veya bos ise refine et)
    $needResolve = (-not $CurrentOs) -or
                   ($CurrentOs -eq 'Unknown') -or
                   ($CurrentOs -match '^Generic')

    # Hostname tabanli refine sadece OS bilinmiyorsa veya Generic ise
    if ($needResolve) {

        # === HOST NAME PATTERNS (en spesifikten en genele) ===

        # VMware ESX
        if ($name -match 'ESX[0-9]|PRDESX|DRCESX|TSTESX|VMWARE|VDIVM|FBPRDVM|FBDRCVM|HYPV|CLESX|EFKEP|LSXLX|BTPRDLX[0-9_]|BTDRCLX[0-9_]|CBPRDLX[0-9_]|CBDRCLX[0-9_]|FEPRDESX|FEDRCESX|FFPRDESX|FFDRCESX|SIMDRCESX|LSXDRCESX|LSXPRDESX|FBPRDVM_|FBDRCVM_|IG_BTPRDLX|IG_BTDRCLX|IG_CBPRDLX|IG_CBDRCLX|IG_LSX[A-Z]+ESX|^EFKEP|^VDIPRDVM|^VDIDRCVM') {
            return 'VMware ESX'
        }

        # Windows
        if ($name -match 'CLN[0-9]|HST[0-9]|EXC[0-9]|HYPVPRD|BNXEAR|BNXSES|EARPRDCLN|BNXMSGPRDEXC|MSGSDRCEXC|FBDRCVM3[6-9]|FBDRCVM4[34]|^ESR|^ALM|^EPR|^COM|^MBE|^YN|^FRA|^FRD|^HST|^INBSES|^ATMPRD|^ATMDRC|^KMP|^KKPRDDB|^KKDRC|^CCPRDCLN|^CCDRCCLN|^MBPRDCLN|^MBDRCCLN|^EPRPRDCLN|^EPRDRCCLN|^TEST$|^ANSIBLE|IG_ATMPRD|IG_ATMDRC|^MB[A-Z]+CLN|^STOPRDAPP06$|WIN[0-9]|^FILE[0-9]|^FS[0-9]|^DC[0-9]|^AD[0-9]|^SQL[0-9]|^IIS[0-9]') {
            return 'Windows'
        }

        # AIX
        if ($name -match 'AIX|^RAC[0-9]|PRDDB[0-9]|DRCDB[0-9]|TSTDB[0-9]|_DB[0-9]|RES[0-9]|ARC[0-9]|MQ[0-9]|^CB|^GEN|^EVA|^TNG|^KFT|^OCM|^CFR|^SEA|^IDM|^VPS|^HCE|^ECM|^IFM|^SCE|^PS|^SD|^OD|^KYS|^BTPRDMGT|^BTDRCMGT|^CSPRD|^CSDRC|^CSTST|^MSPRD|^SFD|^BNXGSC|^BNXMCM|^BNXSW|^BNXMSG|^BNXDTL|^BNXEAR|^BNXEPS|^SWPRD|^SWDRC|^SWTST|^ATL|^IDM[A-Z]+RAC') {
            return 'AIX'
        }

        # Linux
        if ($name -match 'GPU[0-9]|CLW[A-Z]+GPU|KAFKA|KFK[0-9]|^EBS|^SFDPRDRAC[0-9]|^SFDDRCRAC[0-9]|SFDPRDRAC|SFDDRCRAC|FRAP|^IG_FBHWPOC|FBHWPOC|^LSXPRDRAC[0-9]|^LSXDRCRAC[0-9]|^BNXEDPRD|^BNXUCM|^UCMPRD|^UCMDRC|^RAPRD|^STOPRDAPP[1-9][0-9]|^STODRCAPP[0-9]|^STOTSTAPP[0-9]|^IG_STOPRDAPP|^IG_STOTSTAPP|HMRDC|HMTST|^ML[0-9]|^DOCKER|^PGMTSTRAC|SGXLIN|LSXLSN|^LSXPRDLX|ALODRCKFK|ALOPRDKFK|DADRCKFK|DAPRDKFK|FEDWHPRD|FEGNLPRD|^LSXDRCLX|^FRA[A-Z]+CLN|^FF[A-Z]*PRDDB|^FF[A-Z]*DRCDB|FFDRCDB|FFPRDDB|^STOTSTAPP$|^STOPRDAPP$|^TNGPRDAPP|^TNGDRCAPP|^DA[A-Z]+KFK|^ELK|LINUX|RHEL|UBUNTU|CENTOS|^K8S|^POSTGRES|^MYSQL|^MARIADB|^ELASTIC') {
            return 'Linux'
        }
    }

    # === FLAGS (PowerMax specific) - her zaman dene ===
    if ($FlagBlob) {
        $b = $FlagBlob.ToLower()
        switch -Regex ($b) {
            'aix'              { return 'AIX' }
            'vmware|esx|esxi'  { return 'VMware ESX' }
            'win|windows'      { return 'Windows' }
            'linux'            { return 'Linux' }
            'hpux|hp-ux'       { return 'HP-UX' }
            'solaris'          { return 'Solaris' }
        }
        if ($b -match 'avoid_reset_broadcast.*enabled.*true' -and $b -match 'volume_set_addressing.*enabled.*true') {
            return 'AIX'
        }
    }

    # === WWN OUI - sadece OS hala bilinmiyorsa ===
    if ($needResolve -and $WWNs -and $WWNs.Count -gt 0) {
        foreach ($wwn in $WWNs) {
            $w = ($wwn -replace '[^0-9a-f]','').ToLower()
            if ($w.Length -lt 10) { continue }
            # Sik gorulen OUI'ler
            if ($w -match '^20000025b5')  { return 'VMware ESX' }
            if ($w -match '^2000002590')  { return 'VMware ESX' }
            if ($w -match '^10000090fa')  { return 'Linux' }      # QLogic HBA, sik Linux
            if ($w -match '^10000000c9')  { return 'Linux' }      # Emulex, sik Linux/AIX
            if ($w -match '^21000024ff')  { return 'Windows' }
            if ($w -match '^210000e08b')  { return 'Linux' }
            if ($w -match '^c0507607')    { return 'VMware ESX' } # vmware NPIV
            if ($w -match '^c050760')     { return 'VMware ESX' }
        }
    }

    # Hicbiri eslesmediyse mevcut OS'i tut, yoksa Unknown
            ...
            try {
                ...
                if ($allHosts.Count -gt 0) {
                    foreach ($h in $allHosts) {
                        ...
                        # OS resolve - pattern matching + WWN OUI
                    ...
                    foreach ($pName in $policyNames) {
                        try {
                            $sp = Invoke-RestMethod `
                                -Uri "$BaseUrl/replication/symmetrix/$SymID/snapshotpolicy/$pName" `
                                -Method Get -Headers $Headers -TimeoutSec 15 -ErrorAction Stop

                            $intMin  = if ($sp.intervalMinutes)     { [int]$sp.intervalMinutes }     else { 0 }
                            $retSnap = if ($sp.numberOfFlashCopies) { [int]$sp.numberOfFlashCopies } else { 0 }
                            $retDays = if ($sp.secureForDays)       { [int]$sp.secureForDays }       else { 0 }
                            ...
                            if ($sgNames.Count -eq 0) {
                                $Rapor_SnapPolicy += [PSCustomObject]@{
                                    Lokasyon=$Lokasyon; Kabinet=$SymID; 'Policy Adi'=$pName
                                    'LUN / Obje'='—'; 'Host'='—'; 'Periyot'=$intervalStr; 'Saklama'=$retainStr; 'Durum'=$isActive; 'Tip'='PMAX'
                                }
                            } else {
                                foreach ($sgName in $sgNames) {
                                    $Rapor_SnapPolicy += [PSCustomObject]@{
                                        Lokasyon=$Lokasyon; Kabinet=$SymID; 'Policy Adi'=$pName
                                        'LUN / Obje'=$sgName; 'Host'=''; 'Periyot'=$intervalStr; 'Saklama'=$retainStr; 'Durum'=$isActive; 'Tip'='PMAX'
                                    }
                                }
                            }
        ...
        try {
            ...
            if ($aggResp.records) {
                foreach ($agg in $aggResp.records) {
                    ...
                    # Aggregate tipi (SSD/HDD/Hybrid) - block_storage.mirror veya storage_tier'dan
                    $aType = '-'
            ...
            Write-Step "     OK: $TotalTB TB | DR: $DRStr | Agg: $aggCount | SVM: $svmCount | Vol: $volCount | Subs: $SubsTB TB" Green
...
if ($runNetApp) {
    ...
    Write-CSVAndPublish -Data $na.Aggregates -LocalPath (Join-Path $LocalNetApp 'NetApp_Aggregate.csv') -RemotePath (Join-Path $RemoteNetApp 'NetApp_Aggregate.csv') -NoPublish:$NoPublish
